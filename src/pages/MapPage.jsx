...
const MapPage = ({ onBackToMain }) => {
	...
	const handleCheckFacilities = () => {
		if (!userPin) {
			alert(
				"Silakan tandai lokasi terlebih dahulu di peta atau gunakan lokasi saya."
			);
			return;
		}
		// Simulasi fetch data
				...
				<div className="flex items-center">
					{onBackToMain && (
						<button
							...
							className="mr-4 px-3 py-1 bg-brand-light-blue text-brand-dark-blue rounded-md hover:bg-brand-accent transition-colors"
						>
							← Kembali
						</button>
					)}
			...
			<main className="relative flex-grow">
					...
					{!showResults && (
						<>
							<button
								...
								className="w-full py-3 px-4 bg-brand-light-blue text-brand-dark-blue font-semibold rounded-lg shadow-lg"
							>
								Gunakan Lokasi Saya
							</button>
							<button
								onClick={handleCheckFacilities}
								className="w-full py-3 px-4 bg-brand-accent text-brand-dark-blue font-semibold rounded-lg shadow-lg"
							>
				...
				{selectedFacility && (
					<div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-10 p-4 bg-white rounded-lg shadow-xl w-11/12 max-w-sm">
						...
						<p className="text-sm">
							<b>Alamat Lengkap:</b> {selectedFacility.address}
						</p>
						<p className="text-sm">
							<b>Kontak:</b> {selectedFacility.contact}
						</p>
						<p className="text-sm">
							<b>Jam Kerja Instansi:</b> {selectedFacility.hours}
						</p>
					</div>
				)}
