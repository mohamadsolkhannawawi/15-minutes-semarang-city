        ...
        try {
            ...
            if (checkResult.data.exists && checkResult.data.zone_polygon) {
                ...
                setFacilities(formatFacilities(servicesResponse.data));
                 console.log(`📦 [5/5] Ditemukan ${servicesResponse.data.length} fasilitas.`);
            } else {
                console.log("🔄 [3/5] Tidak ada data di DB, mengambil dari ORS...");
                if (!import.meta.env.VITE_ORS_API_KEY) {
                    throw new Error("ORS API Key tidak ditemukan. Pastikan ada di file .env");
                }
                ...
                const newCoords = orsResponse.data.features[0].geometry.coordinates[0].map(
                    ([lng, lat]) => [lat, lng]
                );
                ...
                if (searchId) {
                    ...
                    const servicesResponse = await api.get(`/services/in-zone/${searchId}`);
                    setFacilities(formatFacilities(servicesResponse.data));
                    console.log(`📦 Ditemukan ${servicesResponse.data.length} fasilitas dari data baru.`);
                }
            }
            setShowResults(true);
            ...
            setError(`Gagal memproses data: ${err.response?.data?.message || err.message}`);
        } finally {
            setIsLoading(false);
        }
    };
